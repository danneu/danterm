...
- `references/` -- reproducible external source checkouts; materialize them with `just fetch-references`, consult [agent-docs/reference-sources.md](agent-docs/reference-sources.md), and mine/adapt behavior into DanTerm's neutral fixtures instead of editing the checkouts.
...
- Cite refetchable source trees as `file#identifier`, never `file:line`; use the nearest enclosing named identifier when the point itself is unnamed.
