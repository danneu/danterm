// Unit tests for the reusable trailing-edge dispatch timer debouncer.
...
func debouncerTests() {
    test("Debouncer: cancel before schedule is a no-op") {
        ...
        try expect(!debouncer.isPending)
    }

    test("Debouncer: cancel disarms and stays reusable") {
        // Intent: a scheduled debouncer can be cancelled and scheduled again.
        // Why it exists: pins the cancel/rebuild lifecycle so source retirement
        //   does not make the owning Debouncer one-shot.
        // Scenario: spec-first lifecycle check for search teardown followed by a
        //   later short search needle in the same pane.
        let debouncer = Debouncer(queue: .main)
    ...
    test("Debouncer: rapid reschedule stays cancellable") {
        // Intent: repeated schedule calls re-arm without crashing or losing the
        //   pending state.
        // Why it exists: guards against accidentally resuming an already-resumed
        //   dispatch source while preserving one trailing fire slot.
        // Scenario: spec-first lifecycle check for high-frequency title or cwd
        //   events repeatedly pushing a checkpoint deadline forward.
        let debouncer = Debouncer(queue: .main)
    ...
    test("Debouncer: reschedule moves deadline and keeps newest action") {
        // Intent: a later schedule moves the fire to the later trailing deadline,
        //   runs only the newest action, and clears pending.
        // Why it exists: locks down the central debounce contract against a
        //   fixed-window coalescer or a stale-action reschedule.
        // Scenario: spec-first behavior check for a title-spam burst where each
        //   event must push the checkpoint write until after the last event.
        ...
        try expectEqual(snapshot.0, ["B"])
        try expect(!snapshot.2)
    }
}
