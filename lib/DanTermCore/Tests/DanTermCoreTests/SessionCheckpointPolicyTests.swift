// Deterministic traces for the session checkpoint's coverage and retry rule.
//
...
/// Pins session-checkpoint coverage against failed writes and out-of-order completions.
struct SessionCheckpointPolicyTests {
    private func projection(_ title: String) -> SessionCheckpointProjection {
        var model = makeModel()
        createTab(&model)
        update(&model, .renameTab(id: model.groups[0].tabs[0].id, name: title))
        return SessionCheckpointProjection(snapshot: toSnapshot(model, home: "/Users/testhome"))
    }
    ...
    func failedWriteIsRetried() throws {
        ...
        // Why it exists: the session checkpoint used to advance its baseline at handoff and never
        ...
        var policy = SessionCheckpointPolicy(covering: launch)
        ...
        #expect(try toSnapshot(decodeInitFileCapture(first.capture).model) == changed.snapshot)
        ...
        #expect(try toSnapshot(decodeInitFileCapture(retry.capture).model) == changed.snapshot)
    ...
    func staleFailureLeavesLaterCoverageIntact() throws {
        ...
        // Why it exists: two session writes can be in flight at once, and the older one's
    ...
    func newestFailureRetriesCurrentProjection() throws {
        ...
        let retry = try #require(retryWork)
        #expect(try toSnapshot(decodeInitFileCapture(retry.capture).model) == d.snapshot)
    }
    ...
    func unrestorableProjectionIsRefusedWithoutCoverage() throws {
        ...
        let a = projection("a")
        let b = projection("b")
        let empty = SessionCheckpointProjection(
            snapshot: toSnapshot(makeModel(), home: "/Users/testhome")
        )
        var policy = SessionCheckpointPolicy(covering: a)
        ...
        #expect(try toSnapshot(decodeInitFileCapture(work.capture).model) == b.snapshot)
    }
    ...
    func reversionAfterCaptureBecomesNextWrite() throws {
        ...
        #expect(try toSnapshot(decodeInitFileCapture(reverted.capture).model) == a.snapshot)
    }
}
