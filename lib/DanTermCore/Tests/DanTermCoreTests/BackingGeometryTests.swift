// Swift Testing migration of the legacy `tests/BackingGeometryTests.swift`
// harness suite. Pins pure `backingGeometry(logicalSize:backingSize:)` against
...
@Suite struct BackingGeometryTests {
    @Test("backingGeometry: Retina 2x derives scale and backing pixels")
    func retina2xDerivesScaleAndBackingPixels() {
        ...
        #expect(backingGeometry(
            logicalSize: CGSize(width: 800, height: 600),
            backingSize: CGSize(width: 1600, height: 1200)
        ) == BackingGeometry(xScale: 2, yScale: 2, pixelWidth: 1600, pixelHeight: 1200))
    }

    @Test("backingGeometry: non-Retina 1x derives scale 1")
    func nonRetina1xDerivesScale1() {
        ...
        ) == BackingGeometry(xScale: 1, yScale: 1, pixelWidth: 800, pixelHeight: 600))
    }

    @Test("backingGeometry: derives x and y scale independently")
    func derivesXAndYScaleIndependently() {
        ...
        ) == BackingGeometry(xScale: 2, yScale: 1.5, pixelWidth: 200, pixelHeight: 150))
    }

    @Test("backingGeometry: fractional backing pixels truncate")
    ...
    @Test("backingGeometry: zero logical size returns nil")
    ...
    @Test("backingGeometry: zero backing size returns nil")
    ...
    @Test("backingGeometry: single zero dimension returns nil")
