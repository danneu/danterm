// The pane-tape stream's portable values and its record construction. Finite dumps and
// followed streams both build here, so the two captures cannot drift into separate output
...
/// Adapts one terminal event to this layer without importing terminal modules.
///
/// The recorded event is a type parameter, so the producer's own event value travels through
/// this layer untouched and is written by its own encoder at the wire. Naming the engine's
/// event type here would make every layer below depend on the engine, and lowering it to JSON
/// here would pay a JSON pass per event that the wire encode then pays again.
struct PaneTapeEvent<Event> {
    let sequence: UInt64
...
extension PaneTapeEvent: Equatable where Event: Equatable {}
extension PaneTapeEvent: Sendable where Event: Sendable {}

/// Represents one owner-fenced suffix in protocol-only values for off-main processing.
struct PaneTapeSnapshot<Event> {
    let events: [PaneTapeEvent<Event>]
...
extension PaneTapeSnapshot: Equatable where Event: Equatable {}
extension PaneTapeSnapshot: Sendable where Event: Sendable {}

/// Delivers complete records together with the cursor they advance through.
struct PaneTapeBatch<Event> {
    let records: [PaneTapeOutgoingRecord<Event>]
    let nextCursor: PaneTapeCursor
}

extension PaneTapeBatch: Equatable where Event: Equatable {}
extension PaneTapeBatch: Sendable where Event: Sendable {}

/// Couples the record that opens a stream to the exact cursor its contents continue from.
///
/// It carries no event, so it names no event type: the start record joins the outgoing record
/// family only where it is written.
struct PaneTapeStart: Equatable, Sendable {
    let record: PaneTapeStartRecord
...
struct PaneTapeDump<Event> {
    let start: PaneTapeStart
    let snapshot: PaneTapeSnapshot<Event>
}

extension PaneTapeDump: Equatable where Event: Equatable {}
extension PaneTapeDump: Sendable where Event: Sendable {}
...
func makePaneTapeBatch<Event>(from snapshot: PaneTapeSnapshot<Event>) -> PaneTapeBatch<Event> {
    var records: [PaneTapeOutgoingRecord<Event>] = []
    records.reserveCapacity(snapshot.events.count + (snapshot.droppedEventCount > 0 ? 1 : 0))
    if snapshot.droppedEventCount > 0 {
        records.append(.gap(makePaneTapeExactGapRecord(snapshot)))
    }
...
func makePaneTapeExactGapRecord<Event>(_ snapshot: PaneTapeSnapshot<Event>) -> PaneTapeGapRecord {
    PaneTapeGapRecord(
...
func makePaneTapeDumpRecords<Event>(
    after dump: PaneTapeDump<Event>
) -> [PaneTapeOutgoingRecord<Event>] {
    makePaneTapeBatch(from: dump.snapshot).records + [.end(reason: .dumpComplete)]
}
...
/// neither.
func makePaneTapeEventRecord<Event>(
    _ event: PaneTapeEvent<Event>
) -> PaneTapeOutgoingRecord<Event> {
