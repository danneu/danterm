...
enum Msg {
    ...
    case splitPane(paneId: PaneId, direction: SplitDirection, launch: LaunchSpec? = nil, background: Bool = false)
    case splitFocusedPane(direction: SplitDirection, launch: LaunchSpec? = nil, background: Bool = false)
    case closePane(paneId: PaneId)
    case requestCloseOtherPanes(paneId: PaneId)
    case focusDirection(direction: SplitDirection, side: SplitNodeModel.Side)
