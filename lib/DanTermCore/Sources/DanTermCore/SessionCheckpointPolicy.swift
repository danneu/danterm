// The session checkpoint's write decision: which projection is already covered on disk, and
// what a reported write outcome does to that coverage. It is the sibling of
// `RecoveryCheckpointPolicy`, which schedules the scrollback checkpoint, and it exists for the
// same reason -- so the runtime holds no recovery rule of its own. Timing belongs to the
// runtime: this value never learns that a window was armed or fired, only that a capture was
// asked for.
//
...
/// Decides what the session checkpoint writes, and keeps a failed write retryable.
///
...
struct SessionCheckpointPolicy {
    /// Names one handoff so an outcome can say which write it describes. Two session writes can
    ...
    private var covered: SessionCheckpointProjection?
    ...
    init(covering projection: SessionCheckpointProjection) {
        covered = projection
    }
    ...
    mutating func capture(_ current: SessionCheckpointProjection) -> Write? {
        ...
        return Write(
            ...
            capture: InitFileCapture(sessionProjection: current)
        )
    }
