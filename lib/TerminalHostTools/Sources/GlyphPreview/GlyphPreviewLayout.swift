...
///
/// Which of those scalars DanTerm draws itself is asked of the sprite engine's own
/// vocabulary (`spriteDecode(for:)`) rather than restated here, so the checklist cannot
/// drift from what the renderer actually implements.
struct GlyphPreviewSection {
    let title: String
    let scalars: [Unicode.Scalar]
    let customSpriteScalars: Set<Unicode.Scalar>

    init(title: String, ranges: [ClosedRange<UInt32>]) {
        self.title = title
        let scalars = ranges.flatMap { range in
            range.compactMap(Unicode.Scalar.init)
        }
        self.scalars = scalars
        self.customSpriteScalars = Set(scalars.filter { spriteDecode(for: $0) != nil })
    }
}
