...
func assertCanonical(
    ...
    for (rowIndex, row) in plan.rows.enumerated() {
        ...
        for run in row.backgroundRuns {
            ...
            if let previousBackground {
                #expect(run.startColumn >= previousBackground.startColumn + previousBackground.columnCount, comment, sourceLocation: sourceLocation)
                #expect(run.startColumn != previousBackground.startColumn + previousBackground.columnCount || run.color != previousBackground.color, comment, sourceLocation: sourceLocation)
            }
        ...
        for run in row.overlayRuns {
            ...
            if let previousOverlay {
                #expect(run.startColumn >= previousOverlay.startColumn + previousOverlay.columnCount, comment, sourceLocation: sourceLocation)
                #expect(run.startColumn != previousOverlay.startColumn + previousOverlay.columnCount || run.state != previousOverlay.state || run.color != previousOverlay.color, comment, sourceLocation: sourceLocation)
            }
            previousOverlay = run
        }
        ...
        for run in row.textRuns {
            ...
            #expect(run.cells.allSatisfy { $0.columnWidth == 1 || $0.columnWidth == 2 }, comment, sourceLocation: sourceLocation)
            ...
            if let previousText {
                ...
                #expect(run.startColumn >= previousText.startColumn + previousWidth, comment, sourceLocation: sourceLocation)
                #expect(run.startColumn != previousText.startColumn + previousWidth || run.foreground != previousText.foreground || run.bold != previousText.bold || run.italic != previousText.italic, comment, sourceLocation: sourceLocation)
            }
            previousText = run
        }
        ...
        for run in row.decorationRuns {
            ...
            if let previousDecoration {
                #expect(run.startColumn >= previousDecoration.startColumn + previousDecoration.columnCount, comment, sourceLocation: sourceLocation)
                #expect(run.startColumn != previousDecoration.startColumn + previousDecoration.columnCount || run.kinds != previousDecoration.kinds || run.color != previousDecoration.color || run.strikethroughColor != previousDecoration.strikethroughColor, comment, sourceLocation: sourceLocation)
            }
    ...
    if let cursor = plan.cursor {
        #expect(cursor.row >= 0 && cursor.row < plan.rowCount, comment, sourceLocation: sourceLocation)
