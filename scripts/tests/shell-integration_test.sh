...
prefix="$(printf '\033]1337;DanTermShell=1;%s;' "$token")"
...
bash_output="$(DANTERM_TOKEN="$token" /bin/bash --noprofile --norc -c '
    old_prompt() { :; }
...
fish_output="$(env DANTERM_TOKEN="$token" /usr/bin/env fish --no-config -c '
    source $argv[1]/danterm.fish
    source $argv[1]/danterm.fish
    set -q DANTERM_TOKEN; and exit 31
...
zsh_hooks="$(DANTERM_TOKEN="$token" /usr/bin/env zsh -f -c '
    old_hook() { :; }
    ...
    _danterm_precmd
' _ "$integration_dir" "$command_text")"
bash_hooks="$(DANTERM_TOKEN="$token" /bin/bash --noprofile --norc -c '
    source "$1/danterm.bash"
    _danterm_preexec "$2"
    _danterm_precmd
' _ "$integration_dir" "$command_text")"
fish_hooks="$(env DANTERM_TOKEN="$token" /usr/bin/env fish --no-config -c '
...
precedence_output="$(env DANTERM_TOKEN="$token" LC_DANTERM_TOKEN=wrong /usr/bin/env zsh -f -c '
    source "$1/danterm.zsh"
    danterm_emit_command_end
' _ "$integration_dir")"
test "$precedence_output" = "$prefix""command-end$terminator" || fail "local token did not take precedence"
...
zsh_wrapper_output="$(PATH="$fake_bin:$PATH" DANTERM_TOKEN="$token" /usr/bin/env zsh -f -c '
    source "$1/danterm.zsh"
    ssh -o SendEnv=EXISTING "host name"
    mosh --server="mosh server" example
' _ "$integration_dir")"
bash_wrapper_output="$(PATH="$fake_bin:$PATH" DANTERM_TOKEN="$token" /bin/bash --noprofile --norc -c '
    source "$1/danterm.bash"
    ssh -o SendEnv=EXISTING "host name"
    mosh --server="mosh server" example
' _ "$integration_dir")"
fish_wrapper_output="$(PATH="$fake_bin:$PATH" env DANTERM_TOKEN="$token" /usr/bin/env fish --no-config -c '
    source $argv[1]/danterm.fish
    ssh -o SendEnv=EXISTING "host name"
    mosh --server="mosh server" example
' "$integration_dir")"
for wrapper_output in "$zsh_wrapper_output" "$bash_wrapper_output" "$fish_wrapper_output"; do
    ...
    assert_contains "$wrapper_output" "SSH_ARGS=<-o SendEnv=LC_DANTERM_TOKEN -o SendEnv=EXISTING host name>"
    assert_contains "$wrapper_output" "MOSH_ARGS=<--server=mosh server example>"
done

remote_host="$(/usr/bin/env zsh -f -c 'printf %s "$HOST"')"
remote_output="$(PATH="$fake_bin:$PATH" env LC_DANTERM_TOKEN="$token" USER='user name' /usr/bin/env zsh -f -c '
    source "$1/danterm.zsh"
    [[ -z ${LC_DANTERM_TOKEN+x} ]] || exit 31
...
for remote_shell in bash fish; do
    if test "$remote_shell" = bash; then
        remote_case="$(PATH="$fake_bin:$PATH" env LC_DANTERM_TOKEN="$token" /bin/bash --noprofile --norc -c '
            source "$1/danterm.bash"
            [[ -z ${LC_DANTERM_TOKEN+x} ]] || exit 31
            _danterm_preexec true
            _danterm_precmd
        ...
        remote_case="$(PATH="$fake_bin:$PATH" env LC_DANTERM_TOKEN="$token" /usr/bin/env fish --no-config -c '
            source $argv[1]/danterm.fish
            set -q LC_DANTERM_TOKEN; and exit 31
