...
def hexadecimal_blob(values: list[int], width: int) -> str:
    return "".join(f"{value:0{width}X}" for value in values)
...
enum GeneratedCanonicalCaselessTables {{
    static let decompositionScalars = generatedUInt32Table(
        "{hexadecimal_blob(decomposition_scalars, 8)}"
    )
    static let decompositionOffsets = generatedUInt16Table(
        "{hexadecimal_blob(decomposition_offsets, 4)}"
    )
    static let decompositionPool = generatedUInt32Table(
        "{hexadecimal_blob(decomposition_pool, 8)}"
    )
    static let combiningClassLowerBounds = generatedUInt32Table(
        "{hexadecimal_blob(combining_lower_bounds, 8)}"
    )
    static let combiningClassUpperBounds = generatedUInt32Table(
        "{hexadecimal_blob(combining_upper_bounds, 8)}"
    )
    static let combiningClassValues = generatedUInt8Table(
        "{hexadecimal_blob(combining_values, 2)}"
    )
    static let foldScalars = generatedUInt32Table(
        "{hexadecimal_blob(fold_scalars, 8)}"
    )
    static let foldOffsets = generatedUInt16Table(
        "{hexadecimal_blob(fold_offsets, 4)}"
    )
    static let foldPool = generatedUInt32Table(
        "{hexadecimal_blob(fold_pool, 8)}"
    )
}}
'''
