#!/usr/bin/env python3
"""Build and run the optimized reducer-dispatch cost probe.
...
same-module into the app, so the only way to reach `update()` and `AppModel` is to compile
the probe together with those sources -- which is what this script does and what a package
manifest cannot express. Without `--check` in the gate, the probe is a Swift file no
compiler reads until someone runs the benchmark by hand, and a probe has already gone
stale that way once.
"""
...
PROBE = ROOT / "scripts" / "reducer-dispatch-cost-probe.swift"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="reducer-dispatch-cost-") as directory:
        ...
        binary = scratch / "reducer-dispatch-cost"
        ...
        # way a probe has broken is an API change in DanTermCore, and those are the same
        ...
        subprocess.run(
            [
                ...
                "5",
                "-D",
                "REDUCER_DISPATCH_RELEASE_PROBE",
                "-I",
                str(scratch),
                *shape,
                str(main_source),
        ...
        if arguments.check:
            print("reducer-dispatch-cost: probe type-checks against DanTermCore")
            return 0
