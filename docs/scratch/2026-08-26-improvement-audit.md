...
- [ ] [PROBE-5](#probe-5) -- Make `ResizeProbeRecipe`'s fields `var` and carry `payload` and `sampleCount` into `ResizeProbeReport` (2x5, small, structural)
- [ ] [PROBE-6](#probe-6) -- Return nil on the first unreadable retained row and derive `retainedRowCount` from `storedCellCounts.count` (1x5, small, structural)
- [ ] [PROBE-1](#probe-1) -- Hoist the browse benchmark's plan checksum out of the timed bracket (2x5, small, simplification)
...
- [ ] [SELECT-5](#select-5) -- Delete the unreachable second selection branch in `decidePointerArm`'s `.move` case (1x5, small, simplification)
- [ ] [CHROME-7](#chrome-7) -- Delete `WindowChromeView.updateSeparatorPosition`, which has no caller (1x5, small, simplification)
...
- [ ] [PARSE-2](#parse-2) -- Delete the two capping expressions in `repeatLastPrintedCluster` and loop the requested count through `print` (2x5, small, correctness)
