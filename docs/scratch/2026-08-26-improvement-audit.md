...
- [ ] [UPDATE-2](#update-2) -- Replace the five hand-written teardown rituals with one `tearDownPanes` helper and delete the four dead popover lines (2x5, medium, structural) -- NOT done, redirected 2026-08-31: UPDATE-7's probe fired its frozen rule, so the reconcile-owned ideal is off the table and UPDATE-2 takes its `tearDownPanes` fallback -- one helper called from the five sites in `Update.swift`, plus the four dead popover lines deleted -- as a separate, smaller plan still to be written.
