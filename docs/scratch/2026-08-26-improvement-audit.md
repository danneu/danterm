...
- [x] [PARSE-3](#parse-3) -- Replace both `UInt8(clamping:)` coordinate conversions with one helper that emits xterm's past-end marker (2x5, small, correctness) -- done c8578878, but adjudicated to SELECT-1's answer: suppress, do not mark. The audit is wrong that "no reference terminal" clamps -- it is 5 suppress / 3 clamp / 2 mark, and the clampers are libvterm, tmux and iTerm2. DanTerm's clamp came from libvterm along with the `state-mouse` "Bounds checking" fixture, so this was a recorded decision, not an oversight. xterm's `0x00` marker lost because no child decodes it as one: `tmux/tty-keys.c#tty_keys_mouse` discards any report with a coordinate byte below 0x21, and `fish-shell/src/input_common.rs` underflows it into `invalid_sequence`. Two verified non-consumers, zero consumers -- so the marker is write-only and buys nothing over silence
- [x] [SELECT-1](#select-1) -- Or: range-test above 222 and return `[]`, as ghostty and vte do -- adjudicate against PARSE-3 and land one answer (2x5, small, correctness) -- done c8578878; this is the answer that landed, PARSE-3 closed with it. The guard is `0...222` on both axes, not just an upper bound: negatives clamped to 0x20 and named column -1, which the old assertion never caught. Known cost, accepted: a press in range whose release falls out of range now sends no release, so a legacy child can believe the button is still down. Every suppressor reference has the same hole, so no app that survives ghostty, vte, kitty, foot or Windows Terminal gets worse here. The concrete win is `fish-shell/src/input_common.rs`, which turns a legacy report into `MouseLeft(position)` -- under the old clamp a click at column 300 moved fish's cursor to column 222
...
- [x] [GRID-4](#grid-4) -- Collapse `paintedRow` and `materializedGridRow` into one `materializedRow(at:includeFill:)`, shared with `pullBackOpenTailRemainder` (2x5, small, simplification) -- done d51a2272; the single-pass builder is `GridRow.append(_:scalars:)`
- [x] [GRID-2](#grid-2) -- Put the seam rule and the alternate-screen rule in one private projector that all six readers call (2x5, medium, structural) -- done 36542d8c: `DisplayRowProjector` with active/primary stream factories, all ten hand copies plus the sync encoder go through it, guarded by `display-row-projection-lint.sh`. `scrollbackRow(at:)` now severs the last retained row's wrap under the alternate screen. Left open: the encoder still feeds live grid rows raw, so a blank wide-wrap row loses its seam -- see docs/scratch/2026-08-27-sync-encoder-blank-tail-seam.md
- [x] [SELECT-3](#select-3) -- Hoist `stream[row]` above the column loop in `activationIdentity` and give `explicitLink` a row-scoped cursor (2x5, small, cost) -- done ffe91d36; locates per link went from cell-count to row-count (14 vs 364 -> 3 vs 3 in the pinning test)
...
- [x] [PTY-7](#pty-7) -- Cache the canonical-oversize answer per head record per `c_iflag` while reading tty mode before each write attempt (2x5, small, cost)
...
- [x] [SUPPORT-6](#support-6) -- Split an oversized IPC batch into `ceil(measured/bound)` parts in one step instead of recursive halving (2x4, medium, cost) -- done f3008b26: wall-clock not measured, kept on the 2N cost cap and fewer lines alone
...
- [x] [IPC-2](#ipc-2) -- Pass `state.livenessBound` into `writeHello` and omit `silenceSeconds` when it is nil (2x5, small, correctness) -- done 6be808f6
- [x] [IPC-4](#ipc-4) -- Make `runtimeDispatch` and its init parameter non-optional, with a recording dispatch in the test fixture (1x5, small, structural) -- done 43ccc86f
- [x] [IPC-6](#ipc-6) -- Drop the `text.isEmpty == false` condition in `decodePaneInput` so an empty paste is answered ok (1x5, small, structural) -- done bc9e8784
- [x] [IPC-3](#ipc-3) -- Take `let descriptor = typedRequest.auditDescriptor` once and pass it to both sites (1x5, small, simplification) -- done b7a985a5
...
- [x] [MOBAPP-1](#mobapp-1) -- Narrow the hardware character arm to Ctrl/Alt chords and move the press-to-event decision into `DanTermMobileKit` (4x5, medium, correctness) -- done 04a4dcb0. The decision is `MobileHardwareKeyPress.terminalKey` in the kit, over UIKit-free facts (raw HID usage, `charactersIgnoringModifiers`, `KeyMods`, a Cmd fact), with the 14-row named-key table beside it, so macOS `swift test` covers both. `pressesBegan` is now a thin adapter: a `nil` decision falls through to `super.pressesBegan`, so Shift+A inserts `A` and Shift+2 inserts `@`. One thing the finding did not name went with it: a character with no wire form is sent as text rather than as a chord `KeyName(wireName:)` would reject, which is what keeps a latched accessory Ctrl working on the new text route. The simulator hardware-keyboard pass is done: Shift+A sends one `A`, Shift+2 sends `@`, arrows/Tab/Escape arrive as themselves, Ctrl+C interrupts, and a latched accessory Ctrl then `a` sends 0x01. It has to run in the simulator with Connect Hardware Keyboard on (`Cmd+Shift+K`), which hides the software keyboard; a device run driven by the touch keyboard and the accessory bar tests none of this, because neither reaches `pressesBegan`.
- [x] [MOBKIT-3](#mobkit-3) -- Read the `.end` record in `take` and delete `MobileSessionEvent.recordApplied` with its shell dispatch (3x5, small, simplification) -- done ba50a5f8, as one change with MOBKIT-1
- [x] [MOBKIT-1](#mobkit-1) -- Decode and lift in one function at the model's edge, so both `receive` arms end the connection on an unreadable record (3x5, small, correctness) -- done ba50a5f8 with MOBKIT-3. `take` takes the raw `JSONValue`, so the third failure the finding did not name -- a matching tape reply carrying neither result nor error -- reaches the same verdict through the same function instead of parking the phone on "Connected". The detail is worded for a record rather than an event, since one verdict now covers all three paths.
...
- [x] [MOBAPP-3](#mobapp-3) -- Publish `needsTarget` on `MobileSessionProjection` and reduce `viewDidAppear` to guarding on it (2x5, small, structural) -- done d05e4468, with the other two as one pass. `needsTarget` is stated as "no attempt in flight, and the draft validates to no server", so it reads `MobileTargetDraft.validate()` rather than `draftProblem`: a launch with no host and a draft the model refused answer the same way, and the shell holds no launch rule.
- [x] [MOBAPP-2](#mobapp-2) -- Project the session's menu actions as an ordered typed list and enable the overflow button on it being non-empty (2x5, small, structural) -- done d05e4468. A projected action carries no event -- the shell's exhaustive switch maps each case to its title, symbol, and event together, because the claim and release events are a different type from the new-pane one and a carried event would have had to name both. `canCreatePane` stays on the projection beside the list, unread by the shell, so the model tests can hold the list against the fact it is built from.
- [x] [MOBKIT-2](#mobkit-2) -- Store the prepared outline at the two sites that write the roster, and stop `render` building a second projection (2x5, medium, cost) -- done d05e4468, and further than the finding asked: the raw `[PaneRosterItem]` list is deleted rather than kept beside the outline, and the six one-field shell reads take a `selectedPaneId` accessor instead of the passed-down projection the Wave 14 combined fix proposed. The `.attemptSucceeded` default-pane choice still reads the arriving roster, not the outline -- `isSelectedTab` and `isFocused` are wire facts the outline deliberately drops. No number gates this: the finding's own Correction says the pass is justified structurally.
...
- [x] [UPDATE-7](#update-7) -- Build the headless probe first and expect it to falsify the per-message sweep claim; change nothing until a number says otherwise (1x3, medium, cost) -- done e67cc519, and the number inverted the prediction: the frozen rule (128-tab median at least 1000 ns/dispatch over the 8-tab median) fired at roughly 35x the bound. Across three runs of `just reducer-dispatch-cost`, a title-report dispatch costs ~6.7 us at 8 tabs, ~13.7 us at 32, ~41.4 us at 128 -- a 128-vs-8 delta of ~34.7 us that scales with the tab count. So the sweep is a real per-message cost, the reducer's `defer` takes no new per-message pass, and the item's own non-goal reopens with evidence behind it: `reconcileTabState` builds two `Set<TabId>` per message, and making the canonical check allocation-free is a separate future plan. Plan and full numbers: `plans/impl/2026-08-31-1859-reducer-per-message-budget.md`.
- [x] [UPDATE-2](#update-2) -- Replace the five hand-written teardown rituals with one `tearDownPanes` helper and delete the four dead popover lines (2x5, medium, structural) -- done via the `tearDownPanes` fallback, not the reconcile-owned ideal: UPDATE-7's probe fired its frozen rule on 2026-08-31, so the per-message existence pass stayed off the table. One `tearDownPanes(_:in:cause:)` in `Update.swift` now rejects the pending IPC work and prunes the alerts for a batch of departing panes, and all five sites call it. The four dead popover lines are deleted; `reconcileTodoPopover` in `update()`'s `defer` already retracted a popover whose anchor left the model, and `PaneTeardownTests` pins that premise instead of arguing it. AR1 stands: a future removal path must still remember to call the helper. If UPDATE-7's follow-ups buy the per-message budget back, the ideal reopens.
- [x] [MODEL-6](#model-6) -- Replace the three `model.allPanes` loops with `forEachPane` walks and correct the stale scan-cost comment (1x5, small, cost) -- done 6ce10566. It was four loops, not three: the quit confirmation's running-command rollup flattened the trees too, so it now appends during the walk. A model-level `forEachPane(in: AppModel)` sits beside the node-level one and visits panes in the same tab-then-tree order, and `AppModel.allPanes` is deleted from production, so a future projection cannot reach for the materializer again; tests that want a pane array get the convenience from test support. This landed as a tidy-up with no measurement: the rescore had already settled that the per-`Msg`, ~75 ms-coalesced sweep cadence does not make this a cost win.
- [x] [MODEL-3](#model-3) -- Hold the `SplitNodeModel` root in `ContainerShape` with a payload-skipping compare, deleting `ContainerLayoutNode` and its `Equatable` (1x5, medium, cost) -- done 1743044f. `ContainerShape` holds the tab's own tree, `sameContainerLayout` compares ids, directions and ratios beside `sameContainerStructure`, and `computeContainerOps` asks it where it compared trees with `!=`. `ContainerLayoutNode` and its builder are gone, so the sweep allocates nothing per tab. The finding did not name the guard that came with it: the tree is `fileprivate` and the shape is no longer `Equatable`, so a synthesized `==` cannot silently compare titles, cwds and todos and report a metadata edit as a container change -- payload-inclusive comparison is unavailable by construction. The new tests assert at the diff boundary, not on the helpers. The allocation is still unmeasured; this landed as a structural cleanup, per the entry's own Correction.
- [x] [REFLOW-4](#reflow-4) -- Carry each tracked cursor as a logical offset resolved during the pack walk, deleting the per-cell destination maps (2x4, medium, cost) -- done 65b9625e, d288e098. The landing gate passed. The paired comparison decides on four statistics rather than the two the plan named: the probe alternates narrowing and widening, and on the `wide` recipe the two directions have very different per-resize costs, so a quantile of the combined samples moves between the groups for reasons that are not cost. The rule reads the groups apart -- both narrowing statistics must improve and neither widening statistic may regress -- against thresholds frozen from a two-stage A/A calibration (0.5% narrowing median, 2.0% narrowing p95, 0.5% widening median, 4.0% widening p95, at 24 pairs). The decision run was 24 interleaved pairs in one session on an idle machine, and its verdict is `improved`: narrowing median 2.60 ms -> 0.37 ms, widening median 1.45 ms -> 0.33 ms, with both arms on 10700 retained rows and 1915200 retained cells and a control that moved 0.06% to 0.95%. Artifacts are `docs/scratch/2026-08-31-reflow-4-resize-calibration/decision.json.gz` and `decision-verdict.json`.
- [x] [DRAW-9](#draw-9) -- Delete `nominalGlyph`'s two per-call array allocations, and only then measure a coverage-built glyph table (1x3, medium, cost) -- done 4ccc171c, e9bfa1b2. The path had no workload before this, so the first commit added `symbols-shaped` to the headless draw benchmark and grew its report an absolute per-draw and per-icon-cell cost; the second replaced the `String` and the two arrays in `nominalGlyph` with fixed tuple storage. Measured on 8000 icon cells, full frame, 8 rounds per direction in one session on an idle machine: the deleted allocations are worth -12.55%, or -1.348 ms per draw and -168 ns per icon cell, with a -0.50% order bias, while `btop-shaped` (-0.20%) and `text-shaped` (-0.52%) -- which cannot reach the path -- stayed quiet. The table question the Correction left open is answered and the answer is no: a coverage-built glyph-plus-bounds table, measured as a throwaway arm against the shipped code, buys a further -16.21% (-1.512 ms per draw, -189 ns per icon cell) for 10396 entries at a 48-byte stride, 499008 bytes per `TerminalFontSet` and so per pane and per font size. That is more memory than the Correction's ~340KB estimate for a saving of about 2 us on a prompt with ten icons. Artifacts, the throwaway arm's patch and the reproduction are in `docs/scratch/2026-08-31-draw-9-symbols-measurement/`.
- [x] [DRAW-2](#draw-2) -- Move the eight families' membership predicate into `TerminalSpriteGeometry` and classify a sprite cell as `.band` (3x5, large, cost) -- done 66ef6b87, c6385e08. The eight families' coarse range, `pattern(for:)` and decode tables now live in `TerminalSpriteGeometry` behind one `spriteDecode(for:)` entry, and the executor's routing switch consumes it instead of re-deriving it, so there is one membership answer rather than two. Membership alone does not price a cell: each family declares an ink reach beside its decode, all eight declare `.band` today, and the planner prices a decoded cell at its family's declared reach while everything it cannot decode -- interior gaps of a coarse range included -- stays `.generalText`. The verification pivoted, because the stated one cannot observe the change: the headless draw arm plans once outside the timer, `drawRenderFrame` never reads `inkClass`, and the clip band is driver-chosen, so `--clip-rows 1` measures nothing this touches. What replaces it is deterministic: a damaged sprite row's apply shape shrinks to one band's erase span and a correspondingly smaller plan set, against a pure-sprite streaming-and-editing scenario through `TerminalFrameBackingStore.apply` that stays byte-identical to full redraws and was proven non-vacuous before the planner changed. Wall clock is descriptive only and claims no verdict: two `just benchmark-quick 66ef6b87 incremental-mixed` runs read +17.1% and +16.2% at 2 pairs where the frozen rule needs 40, on a workload whose content is pure ASCII with no sprite cell in it -- re-measure at the ladder's pair count on an idle machine before reading anything into it. Follow-up left open: `lib/TerminalHostTools/Sources/GlyphPreview/GlyphPreviewLayout.swift` still hand-writes the eight families' ranges as `implementedRanges` and now duplicates the shared vocabulary.
...
- [x] [PERSIST-1](#persist-1) -- Make `last-light.json` the only structure on disk, written on the exit path, with `last-enriched.json` a scrollback-only sidecar (3x5, medium, correctness) -- done ebb87640, 1ef680a3, a8cff28a. The two files are `last-session.json` (structure) and `last-scrollback.json` (text), and the "light" and "enriched" names are retired: `LightCheckpointPolicy` is `SessionCheckpointPolicy`. The empty-model quit does not regress, because the first commit landed the half neither proposal had: one restorability predicate that both writers consult, so a session the next launch would refuse is never persisted in the first place.
...
**Verified.** The ideal fix landed, and the verification this entry states did
not survive contact with the harness. The paired headless draw arm plans once
outside the timed bracket, `drawRenderFrame` never reads `inkClass`, and the
clip band is chosen by the driver rather than by the ledger, so
`--clip-rows 1` cannot resolve a change to how a row is priced. The evidence is
the deterministic shape instead: a damaged sprite row's apply shape shrinks to
one band's erase span and a smaller plan set, and a pure-sprite streaming and
editing scenario through `TerminalFrameBackingStore.apply` stays byte-identical
to from-scratch full renders -- written and run green before the planner change
so the gate is not vacuous. The Correction's scope estimate held: all eight
`pattern(for:)` functions and their tables moved. Wall clock stays descriptive
(`incremental-mixed` at 2 pairs, on ASCII content this change cannot reach).
...
**Measured.** The narrow half landed and the table half is refused on its own
numbers. Deleting the per-call allocations is worth -12.55% of the
`symbols-shaped` draw; adding the coverage-built table on top of that is worth a
further -16.21%, for 499008 bytes per `TerminalFontSet`. The full readings, the
throwaway arm's patch and the reproduction are in
`docs/scratch/2026-08-31-draw-9-symbols-measurement/`.
...
**Done.** Took the Correction's recommendation, not the ideal fix. UPDATE-7's probe measured the per-message sweep at ~35 us of a ~41 us title report at 128 tabs, which fired its frozen rule, so the existence pass this item's ideal wanted was ruled out on cost before it was written. What landed is the cheaper fallback: one `tearDownPanes(_:in:cause:)` in `Update.swift` that rejects a batch of departing panes' pending IPC work and prunes their alerts, called from all five sites. The `PendingIpcRejectionCause` wordings survive unchanged, which the ideal would have lost. The four dead popover lines are deleted; `PaneTeardownTests` drives each removal path and asserts the reconcile sweep retracts the popover on the same message, so the load-bearing premise is pinned rather than argued. The `defer` block is untouched, so MODEL-5's dependency on it needs no re-check. The trade-off is on the record: a future removal path must still remember to call the helper, and only a cheaper per-message budget reopens the ideal. Plan: `plans/impl/2026-09-01-0944-teardownpanes-helper.md`.
