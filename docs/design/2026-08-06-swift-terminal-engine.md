...
| D3 | Private Use Area glyphs, including Nerd Font glyphs, are ordinary font glyphs. No separate Nerd Font protocol is required. When the configured font does not map a PUA scalar and the packaged symbols face does, the renderer starts that face at the configured base font size. For each nonempty glyph, it measures the ink bounds, applies one isotropic scale equal to the smaller of the horizontal fit, vertical fit, and 1, and centers the fitted ink in the planned cell span. An empty glyph stays blank. The span clip remains the final containment boundary. *Amended 2026-08-24:* packaged symbols previously used cell width as their point size and drew at the normal baseline, which made branch icons short and left other glyphs clipped or off-center. | live |
...
| D7 | Primary-screen resize reflows retained history and active rows as one ordered sequence of logical lines. Each retained record is one whole logical line, and its display fold is a function of `(line, width)` only; chunk boundaries and the arena wrap cannot create row boundaries. Trailing never-written blank rows are layout data and keep their count across width changes; wrap growth displaces rows into history instead of consuming those blanks, and widening pulls the displaced rows back before adding new padding. The history/live seam is derived from that one stream, so an unchanged width round trip restores the layout without resize-series state. Cursor visibility bounds blank preservation: if the reflowed cursor would land above the viewport, resize removes only the trailing blanks needed to show it. Explicit browsing keeps its content anchor, and a bottom-row cursor remains the zero-trailing-blank bottom-follow case. This follows the structural model used by alacritty, tmux, and iTerm2. Resize runs in two ordered stages: the live cursor alone decides the resulting layout, and the DECSC slot is then mapped through the displacement or reflow that layout produced, by the same attachment rules. *Amended 2026-08-27 (Wave 7 of the 2026-08-26 audit):* the fold carries every fact of a row with a visible effect. A hard-ended row folds to its visible extent -- content cells, then one trailing fill style to the margin, the same rule history admission measures by -- and the fill paints every blank the fold synthesizes for the line, so a width change keeps the paint a background erase left. The live cursor's row also folds the blanks between its extent and the cursor, so the cursor keeps its distance from the text at every width; a row those blanks open exists for the cursor alone and comes out of the trailing-blank budget, not the content. Two anchors and no third: a cursor on a cell the fold carries follows that cell with no pending wrap; any other cursor follows the boundary after its row's units, spelled as the last column plus a pending wrap only when the boundary falls on the new margin. There is no column clamp. The DECSC slot is a passenger: it follows its cell while its cell is in the fold, its line's end when it sat past the fold bound (it never opens a row), and its row offset below the content when its row is a never-written blank below the last visible effect; a saved row that left the active area takes the live cursor's off-screen policy (row 0, column kept). The height-shrink trim drops text-blank rows by the gated wrap claim (`logicallyContinues`), never the raw one. *Amended 2026-08-13:* the resize-series fallback preserved a remembered layout only across selected consecutive events; preserving blank rows makes each width change information-preserving by construction. | live |
...
| E7 | Export and scrollback recovery checkpoints capture a bounded positional tail of primary-screen history. DanTermCore supplies a 4,000-line and 399,999-grapheme content limit; TerminalCore applies that one cut at canonical hard boundaries without trimming whitespace or adding a final newline. Alternate-screen content is transient and is never replayed into a new shell. *Amended 2026-08-24:* the engine previously over-read a suffix for a second DanTermCore budget cut. | live |
| E8 | DanTermCore owns checkpoint retention policy and stored normalization: it reserves one grapheme for the stored newline, trims leading and trailing whitespace from the bounded engine result, omits the result when nothing remains, and otherwise stores exactly one final newline within the 400,000-grapheme bound. Boundary whitespace can now consume positional budget before normalization, and an over-budget logical line with no hard boundary in its kept window is omitted. *Amended 2026-08-24:* the earlier trim-before-cut pipeline preserved those two legacy edge cases by applying the budgets a second time downstream. | live |
...
| id | Decision | Status |
|---|---|---|
| G1 | macOS owns text composition on every native Option side. Configuration can instead assign the left Option key, right Option key, or both to terminal Alt. An assigned side translates a text-producing key without Option, then submits that key with terminal Alt through the active keyboard protocol. Fixed and non-text keys keep their terminal identity and carry Alt whenever Option is held. *Amended 2026-08-24:* Option was previously always native and Option-as-Alt did not exist. | live |
| G2 | Native Option sides keep macOS composition precedence over every terminal keyboard mode. Active composition and text that cannot represent one modified terminal key stay on the committed-text path. A configured terminal-Alt side bypasses native Option text only when translation identifies one terminal character; Kitty encoding then applies to that key and its terminal modifiers. *Amended 2026-08-24:* composition previously had precedence on every Option side because no side could become terminal Alt. | live |
...
| G6 | The engine supports local text selection and SGR mouse reporting. A settled selection is anchored: an anchor unit, a focus boundary, and the granularity, on one lifetime. A press sets the anchor, a Shift press keeps it, and everything moves the focus, so a plain click leaves an invisible caret and the Shift-click that follows selects anchor-to-click -- inside the old range, outside it, or across the anchor alike. A press the child receives takes the local selection away, because it is the only press the user has left. Only the plain press keeps its caret: a caret a Shift press mints ends with its own gesture, because it is one the user could not see to aim. When an application has captured mouse input, Shift-left bypasses reporting and runs the same rule with one departure: a Shift multi-click replaces the selection with the token or line under the pointer instead of extending, because the anchor it would extend from is one no plain click could have aimed. | live |
...
| G8 | With application mouse reporting active, an unmodified wheel event goes to the application and does not scroll local history; Shift-wheel forces local history navigation and emits no mouse-report bytes; the native scrollbar is always local and never emits terminal mouse input. **With mouse reporting off and the alternate screen active, alternate scroll (DEC mode 1007) decides the unmodified wheel:** while it is set -- the default, matching ghostty and foot -- the wheel reaches the child as cursor keys, and while the child resets it the wheel stays local, which over an alternate screen scrolls nothing. *Amended 2026-08-27:* the alternate-screen wheel was previously unconditional, so a child that reset the mode still received synthetic cursor keys. | live |
...
| H7 | **DanTerm JSON at `~/.config/danterm/config.json` is the sole configuration authority.** The engine reads no config of its own and no other project's config or theme files. The known schema includes theme, font, UI, keyboard, shell-launch, tailnet, and keybinding settings; `keyboard.optionAsAlt` selects physical Option sides for terminal Alt. *Amended 2026-08-24:* the earlier field list was incomplete and predated Option-as-Alt. | live |
...
| I5 | XTVERSION accepts `CSI > q` and `CSI > 0 q` and replies `DCS >|DanTerm <version> ST`, using the same injected bundle version exported as `TERM_PROGRAM_VERSION`. DECRQSS answers `DCS $ q` over exactly SGR, DECSTBM, DECSCUSR, and DECSCA, and never echoes a request it rejects. XTGETTCAP answers `DCS + q` from a generated projection of `docs/terminal-capabilities.md`, under xterm's prefix semantics, and never echoes a name that misses. **DA2 and 8-bit replies remain unsupported.** | live |
