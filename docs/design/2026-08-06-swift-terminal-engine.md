...
| G6 | The engine supports local text selection and SGR mouse reporting. A settled selection is anchored: an anchor unit, a focus boundary, and the granularity, on one lifetime. A press sets the anchor, a Shift press keeps it, and everything moves the focus, so a plain click leaves an invisible caret and the Shift-click that follows selects anchor-to-click -- inside the old range, outside it, or across the anchor alike. A press the child receives takes the local selection away, because it is the only press the user has left. When an application has captured mouse input, Shift-left bypasses reporting and runs the same rule. | live |
