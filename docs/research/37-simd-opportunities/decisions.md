...
- Decision: keep rank 1 and rank 4; reject ranks 2 and 6 for want of a workload;
  hold rank 5.
...
Ranks 2 (`eraseCells`) and 6 (`moveAndFillCells`) are rejected, not deferred. Neither
...
Rank 4 (`admissionExtent` -> maintained `contentEnd`) is kept, and `F3` since sized it
at 5.04% of `scrollback-stream` CPU -- larger than either `appendCells` hoist alone.
It is a deletion, not an acceleration, so `F1` section 4's rule prefers it regardless.
