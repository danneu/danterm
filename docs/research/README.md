...
| 28 | [Retained-row optimizations](28-retained-row-optimizations/README.md) | Follow-on wins opened by compact retained rows | C6 landed at 14.1x depth; `D8`'s cell+row caps fixed resize, then the deciding gate failed on browsing (+19.83%); H3 open pending `F17`'s profile |
