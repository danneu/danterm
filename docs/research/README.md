...
| 15 | Memory footprint | **LIVE.** Takes up doc 12's recorded reopening condition. Owns resident bytes per cell, per row, and in aggregate. **Phase 1 closed; Phase 2's engineering half shipped; Phase 3 has taken every hypothesis it opened.** `15/F4`+`15/D1`: evicted scrollback rows kept their cells, so history held up to **2x** the rows it admitted (~22 MB at peak), fixed at no CPU cost. `15/F7` closed the attribution -- holding 21.75 MB of cells costs ~25 MB (cells, plus 1,488 B/row of malloc bucket rounding, plus ~4 MB allocator slack, plus **zero** retention). `15/D2`+`15/F9`: the byte budget charged 40 bytes for a 72-byte cell, so a 10 MB budget held ~22 MB; corrected, it holds ~10.8 MB and history halves from ~1,704 to ~810 rows. Two of Phase 1's four answers were corrections to its own instruments: `15/F6` (`benchmark-memory` cannot resolve a representation change) and `15/F7` (the headless probe charged its own feed call to resident state). `15/D3`+`15/F10` took `H2`: the cell is **72 -> 56 bytes**, because `hyperlinkId`'s cost was its `Int?` *alignment*, not its width -- narrowing it to `UInt16?` recovered all 16 bytes and made libghostty's side map unnecessary. `scrollback-stream` is **18-19% faster** (reproduced twice against an A/A control; contrast `12/F8`, which rejected the POD cell for making that workload 6.7% slower), and per-row bucket overhead fell 1,487 -> 255 bytes. At a fixed byte budget it buys **history, not memory**: ~10.6 MB either way, 876 -> 1,107 rows. `15/F11` then answered doc 12's open question about `H3` without implementing it: style **writes** run 9-23 million per corpus against 1-5 distinct **values**, so a refcounted dedup table would charge on the only hot side -- `H3` should sweep a live set the way `hyperlinkTargets` already does, which costs nothing per cell write. Worth 16 more bytes (stride 56 -> 40), the largest win left, though the malloc bucket returns about a third of it. `15/F12` then implemented, measured, and **reverted** `H4`'s cheap half: narrowing `contentIdentity` to `UInt32?` reaches stride 48, is worth **-26%** at 80 columns, and **costs 1.8 MB** at 179 -- the row never leaves its malloc bucket there, while the budget charges `columns * stride` and so admits 15% more rows at unchanged cost. Between ~43 and 56 bytes a 179-column row costs the same, so at wide geometries the row allocation has replaced the cell as the lever. `15/D4`+`15/F13` then closed the budget's last systematic gap: `scrollbackByteCost` charged what a row's cells *occupy* and now charges what the row *reserves*, by reading `Array.capacity` -- the allocator's own answer to the bucket-rounding question `15/F7` correctly refused to model. History lands within **~1%** of the configured 10 MB at both 179 and 80 columns, against 8-17% over before, at no CPU cost; depth falls 1.5% at 179 and 10.7% at 80. `15/F14` then **retook the change `15/F12` reverted**, unchanged, because `15/D4` changed what the budget charged for it: the cell is **48 bytes**, at 80 columns the pane holds **24% more history for 18% less memory**, and at 179 columns it is neutral in both directions where the identical patch cost 1.8 MB before. A 32-bit identity is genuinely exhaustible, so the wrap skips zero and drops any armed link rather than let a reissued identity satisfy a Cmd-click armed before it. `15/F15` then took `H3`, Phase 3's last hypothesis: `TerminalStyle` left the cell for a 32-bit id into a swept table, and the cell is **32 bytes -- 56% of where this file started**. It is the **first change here that pays at 179 columns**, where every earlier shrink landed inside the same malloc bucket and bought nothing: per-row overhead falls **1,698 -> 456 bytes** and the pane holds **62% more history for 7% less memory**. Two things were not in the hypothesis as written -- Swift lays out stored properties in declaration order, so moving the only 8-byte-aligned field first is worth a whole bucket on its own (48 -> 48 alone, but 40 -> 32 combined); and the id is 32-bit rather than doc 12's proposed 16-bit, because 16 bits caps the terminal at 65,536 live styles and a truecolor image renderer assigns a distinct RGB per cell, exhausting that in a few screens with no fallback but approximated colors. The draw-path lookup it was expected to pay for **did not appear**: plan time fell **6-9%** on all three draw workloads, because interning moved off the cell-write path onto the SGR pen, so the ~30 write sites store four bytes where they used to copy nineteen, and `terminal-feed` is **18-19% faster** across two independent runs (-19.04%, -18.32%). At 80 columns the trade inverts mildly -- +31% history for +5-7% footprint -- because `48 + 80 * 32` sits just past a size class, which sharpens rather than settles `15/D2`. `15/D2` is now **closed by the user: the budget stays denominated in bytes, at 10 MB.** Limiting by memory capacity beats limiting by lines, which bounds only a proxy -- 2,000 lines is 4 MB at 80 columns and 11 MB at 179, and whoever set it cannot see that. The accepted price is the one this file paid three times over: with a byte budget, every representation win silently changes history depth. No code change -- it confirms the status quo, and the nominal value needed no raising, because the depth problem was a cell-size problem all along: `15/D2`'s accounting fix dropped 10 MB from a dishonest ~1,704 rows to an honest ~810, and `15/F10`+`15/F14`+`15/F15` then took the cell 72 -> 32 bytes and put **1,768 rows** back inside the same honest 10 MB. Caveat on record: the budget bounds *charged* bytes (equal to reserved bytes since `15/D4`), not process footprint -- at 80 columns an honest 10 MB now costs ~11.5 MB resident. `15/F16` records the settled cell's measured field-by-field layout so it need not be re-derived, and one thing that falls out of it: after four rounds of narrowing, **padding is the largest single line item in the cell** -- 10 of 32 bytes, 3.02 MB at a full 179-column history, more than any real field. Seven of those are tail padding forced by `TerminalScalars`' 8-byte alignment; the interior three cannot be recovered by reordering (25 is the floor for these types). Also priced: `hyperlinkId` costs 0.91 MB for a feature `15/F2` measured as `nil` in **100% of cells on every payload**. Two candidate directions are recorded without being promoted to hypotheses -- a 4-byte-aligned `TerminalScalars` (stride 28) and moving `hyperlinkId` to a side map (size 21) -- because since `15/F10` it is the malloc bucket, not the stride, that decides whether a cell shrink reaches the user, and 32 -> 28 is a far smaller step than any taken so far. Remaining: **Phase 4 close-out** only. |
