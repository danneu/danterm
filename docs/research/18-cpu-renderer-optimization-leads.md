...
| `L15` | **Metal glyph atlas.** | `docs/design/2026-08-06-swift-terminal-engine.md` defers it by design (`H1` makes Metal a possible later renderer rather than a dependency, `L8` rejects a first-version GPU renderer, and `H12` accepts the CPU cost that follows), and `F3` notes it is what every fast terminal did. Out of scope for a *CPU renderer* file; recorded so the ranking is not mistaken for a claim that the CPU path can win outright. |
