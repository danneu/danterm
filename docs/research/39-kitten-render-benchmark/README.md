...
- [ ] Headless: add the four kitten streams as `terminal-feed` corpora, or as a
  sibling headless workload `kitten-feed` with one block per arm, so `H1`-`H4`
  get a verdict without a window. Enters as a `CANDIDATE_WORKLOADS` entry with
  an A/A screen before any rule is frozen
  (`scripts/terminal-benchmark-validation.py`). VETTING
- [ ] A/A series and a frozen threshold for whichever arm graduates, per
  [agent-docs/measurement-discipline.md](../../../agent-docs/measurement-discipline.md).
  A human freezes it. TODO
