...
| # | Fix | Why | Done |
|---|---|---|---|
| F1 | Raise `ResizeProbeRecipe.sparseSaturating.lineCount` 250,000 -> 500,000 and give the guard an explicit margin bound | Discovered by row #11: the shipped recipe saturates by **1.005x** (implied depth ~248,700 vs 250,000 lines) and the guard asserts only `impliedDepth <= lineCount`, so it cannot tell 1.003x from 3x. An unrelated charge-rate change would silently make doc 28's sparse numbers stop meaning saturation. Row #11 decoupled the guard's cost from `lineCount`, so only the manually-run probe pays for the increase. Verify empirically that the probe's reported numbers do not move -- once budget-bound, retained depth should stay ~248,700 either way. | [x] Margins now dense 2.85x / sparse **2.01x** / wide 5.40x against a `impliedDepth * 1.5 <= lineCount` bound (tightest headroom 1.34x, and the measurement is exact integer arithmetic, so nothing to flake). Retained state verified unmoved: 250,000 lines -> 246,595 records / 13,564,656 arena bytes, 500,000 -> 246,596 / 13,564,592 (0.0004%/0.0005%), so doc 28 needs no regeneration. Guard 1.511s -> 1.557s, suite 9.35s -> 8.90s. |
| F2 | Bound `allocateHyperlinkId`'s scan by the id space so it cannot spin | Row #3 established that removing the `targets.count > HyperlinkId.max` guard makes `allocateHyperlinkId` spin forever, so the test **hangs instead of failing** under exactly the neutralization this plan's discipline calls for. It already cost one leaked run that pinned a core for 95 minutes. **`.timeLimit` was the planned fix and does not work**: it cancels a task, Swift cancellation is cooperative, and a synchronous loop never checks it -- measured, a 60s limit had not fired at 130s. Bounding the scan makes the function total instead, so the hazard is gone at the source and either mechanism can be neutralized alone without hanging. | [x] |
