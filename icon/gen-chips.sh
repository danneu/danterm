...
w('}')
w('')
w('/// The two states of a chip in a tab row\'s pane strip, plus the three colors')
w('/// its two state dots can take.')
w('public struct ChipPaneListPalette: Sendable {')
w('    public let inactive: ChipPalette')
w('    public let active: ChipPalette')
w('    /// A pane with an unread alert, marked at the chip\'s top-trailing corner.')
w('    public let alertDot: CGColor')
w('    /// An agent blocked on a prompt, marked at the bottom-trailing corner.')
w('    /// Green rather than red so it can never be read as the alert above it.')
w('    public let waitingDot: CGColor')
w('    /// An agent mid-turn, at the same bottom-trailing corner as `waitingDot`.')
w('    /// The same dot in a different hue -- see the stateDot note in chips.json.')
...
w('/// value for every mark, so none can lose the ring the others keep.')
...
for mode in ('light', 'dark'):
    ...
    w(f'        alertDot: {color(m["stateDot"]["alert"])},')
    w(f'        waitingDot: {color(m["stateDot"]["waiting"])},')
...
w('')
w('    /// State-dot geometry, in points at `paneRowSize` and scaled with it.')
w('    /// The alert mark and the agent mark share it and differ only in hue and')
w('    /// in corner, so the ring is a property of the dot rather than of one')
w('    /// state. See the stateDot note in chips.json.')
w(f'    public static let stateDotGeometry = ChipStateDotGeometry('
  f'diameter: {fmt(sd["diameter"])}, ringWidth: {fmt(sd["ringWidth"])})')
w('    /// How far a dot overhangs its corner of the chip. Lands in margins the')
w('    /// strip already has, so it stays out of the strip\'s fitting math.')
